...
class SpyMessages:
    ...
    def assert_number_unique_messages(self, expected_number_messages: int):
    ...
    def get_counter_by_message_id(self, message_id: Uuid):
        return self.counter_messages.get(str(message_id), {}).get("counter", 0)

    def assert_count_by_message_id(self, message_id: Uuid, expected_count: int):
        assert (
    ...
    def assert_first_message(self, expected_message: Message):
        self.assert_number_unique_messages(1)
        event = self.messages[0]
        assert event == expected_message

    def assert_last_message(self, expected_message: Message):
        self.assert_number_unique_messages(1)
