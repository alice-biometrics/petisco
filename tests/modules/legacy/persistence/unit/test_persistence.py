...
from petisco.legacy import Persistence, FakeDatabase
...
def test_should_execute_lifecycle_of_persistence_with_fake_database():
    ...
    assert (
        "tests.modules.legacy.persistence.ymls.object.models.UserModel"
    ...
    assert (
        str(persistence)
        == "Persistence: {'fake': {'name': 'fake', 'models': {'user': 'tests.modules.legacy.persistence.ymls.object.models.UserModel', 'product': 'tests.modules.legacy.persistence.ymls.object.models.ProductModel'}}}"
    )
